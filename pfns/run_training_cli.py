...
def parse_args():
    ...
    parser.add_argument(
        "--pregenerated-batches-dir",
        type=str,
        default=None,
        help=(
            "Directory with batch_*.pt files or one stacked .pt file generated "
            "from pregenerated batches."
        ),
    )

    parser.add_argument(
        "--pregenerated-batches-no-shuffle",
        action="store_true",
        help="Read pregenerated batches in sorted order instead of shuffling them.",
    )
...
def main():
    ...
    if args.pregenerated_batches_dir is not None:
        from pfns.priors.pregenerated_batches import PregeneratedBatchPriorConfig

        config = config.__class__(
            **{
                **config.__dict__,
                "priors": [
                    PregeneratedBatchPriorConfig(
                        data_dir=args.pregenerated_batches_dir,
                        shuffle=not args.pregenerated_batches_no_shuffle,
                    )
                ],
            }
        )
