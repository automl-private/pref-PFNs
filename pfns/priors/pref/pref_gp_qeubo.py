...
def get_batch(
    ...
    **kwargs,
    ...
    # Each original GP input has dimension gp_dim.
    ...
    for t in range(seq_len):
        ...
        if t < single_eval_pos:
            ...
            # qeubo[:, t] stays 0 for context positions
        else:
            # Query: keep original random order, target is max(f0, f1)
            # from noiseless GP.
            f0 = Fs[:, i0]  # (B,)
            f1 = Fs[:, i1]  # (B,)
...
class PrefGPqEUBOPriorConfig(PriorConfig):
    # Scalar -> isotropic RBF.
    # Sequence / tensor of length gp_dim -> ARD RBF.
