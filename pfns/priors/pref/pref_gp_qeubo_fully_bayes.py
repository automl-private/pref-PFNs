...
def get_batch(
    ...
    **kwargs,
    ...
    # Build a batched GP prior: batch dimension indexes independent tasks.
    mean_module = gpytorch.means.ConstantMean(batch_shape=torch.Size([batch_size]))
    mean_module.constant = torch.full((batch_size,), float(mean_constant), device=device)
    ...
    with torch.no_grad():
        ...
        Fs = f_dist.rsample()
        Ys = Fs + noise_std.reshape(batch_size, 1) * torch.randn_like(Fs)
