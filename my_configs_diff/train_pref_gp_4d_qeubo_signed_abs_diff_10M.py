...
from pfns.priors.pref.pref_gp_qeubo_signed_abs_diff import PrefGPqEUBOSignedAbsDiffPriorConfig

gp_dim = 4
num_features = 4 * gp_dim
...
prior = PrefGPqEUBOSignedAbsDiffPriorConfig(
    lengthscale=0.2,
    outputscale=1.0,
    noise_std=0.05,
)
...
config = MainConfig(
    ...
    warmup_epochs=100,
    train_state_dict_save_path=f"checkpoints_diff/pfn_pref_gp_{gp_dim}d_qeubo_signed_abs_diff_10M.pt",
    train_state_dict_load_path=f"checkpoints_diff/pfn_pref_gp_{gp_dim}d_qeubo_signed_abs_diff_10M.pt",
    validation_period=5,
    verbose=True,
    progress_bar=False,
    tensorboard_path=f"tb_diff/pref_gp_{gp_dim}d_qeubo_signed_abs_diff_10M",
    num_workers=0,
)
